...
public class SafetyAccessibilityService extends AccessibilityService {
  ...
  private void apply(){
    if(!getSharedPreferences("safety",0).getBoolean("enabled",false)){remove();return;}
    if(overlay==null){
      WindowManager wm=(WindowManager)getSystemService(WINDOW_SERVICE);
      overlay=new FrameLayout(this);
      WindowManager.LayoutParams lp=new WindowManager.LayoutParams(-1,-1,WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY,WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE|WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE|WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN,-3);
  ...
  private void remove(){if(overlay!=null){((WindowManager)getSystemService(WINDOW_SERVICE)).removeView(overlay);overlay=null;}}
